...
// Encode an 8-bit RGB triplet into RGBA5551 (alpha bit always set). This is the
// exact same packing the N64 RDP expects for a 16-bit TLUT entry.
static inline uint16_t conv_rgba5551(uint8_t r, uint8_t g, uint8_t b) {
...
// Bilinear box-scale an RGBA frame to (dw x dh). Returns a freshly allocated
...
static void median_cut(ucolor_t *uni, int nuni, int ncol, uint16_t *tlut, int *uni2pal)
{
	...
	for (int i = 0; i < (int)boxes.size(); i++) {
		...
		tlut[i] = (uint16_t)((r5 << 11) | (g5 << 6) | (b5 << 1) | 1);
	}
}
	...
	uint8_t *idx, uint16_t *tlut)
{
	...
	for (int i = 0; i < npix; i++)
		c[i] = conv_rgba5551(rgba[i*4+0], rgba[i*4+1], rgba[i*4+2]);
...
int gif_convert(const char *infn, const char *outfn)
{
	...
	// Per-frame GCE delays (ms), clamped so they never collide with a sentinel.
