 ...
 * @endcode
 *
 * Each frame is composited (GIF disposal resolved) and scaled to width x height
 * before quantization, so the stream is a self-contained flip-book: no
 * inter-frame state is needed to display frame N. Per-pixel GIF transparency is
 * preserved in each TLUT entry's RGBA5551 alpha bit.
 */
