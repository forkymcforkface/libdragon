...
## Why CI8 is lossless for GIF

A GIF frame is, by the GIF format's own definition, a palette image with **at
most 256 colors**. A CI8 index plane + 256-entry TLUT therefore reproduces an
un-scaled frame **exactly** — CI8 is not a quality reduction here, it is the
natural fit, at half the bytes of a 16-bit RGBA5551 frame.

The only time a frame loses fidelity is when it must be **scaled** to a size the
source isn't already at (bilinear resampling can introduce more than 256 distinct
colors in a frame), or when `--colors` is set below the frame's real color count.
In those cases the frame is reduced to the requested palette with a pixel-weighted
**median cut**, and the file's `GIF64_FLAG_LOSSY` flag is set. Frames already at
the target size stay lossless. `-v` reports how many frames were quantized.
...
// then, repeated nframes times:
    ...
    uint16_t tlut[256]            // RGBA5551 palette (alpha bit always set)
```

Each frame is self-contained (GIF disposal and transparency are composited
during conversion), so displaying frame *N* needs no other frame.
