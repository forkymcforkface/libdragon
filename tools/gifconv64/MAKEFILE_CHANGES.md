...
`gifconv64` is a single translation unit: `gifconv64.cpp` includes the vendored
`binout.c` and textually includes `conv_gif64.cpp` (which pulls in the
`stb_image.h` implementation), so only one object file is produced.
